...
import {
  LayoutDashboard, BookOpen, Users, ArrowRightLeft,
  ArrowLeftRight, Banknote, Bookmark, BarChart3,
  UserCog, Library, ChevronRight, Moon, Sun, Menu, X, LogOut, ChevronLeft, Settings,
} from 'lucide-react'
...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    if (savedCustom) {
      try {
        ...
        if (c.brand) root.style.setProperty('--brand-light', hexToRgb(c.brand, 0.08))
      } catch (e) { /* ignore parse errors */ }
    }

    if (savedTextSize) {
      const TEXT_SIZES_LOCAL = { small: 0.9, normal: 1, large: 1.1, xlarge: 1.2 }
  ...
  const Sidebar = ({ collapsed = false }) => (
    <aside
      style={{ width: collapsed ? 'calc(var(--sidebar-w) * 0.3)' : 'var(--sidebar-w)' }}
      className="flex-shrink-0 flex flex-col h-full transition-all duration-500 ease-in-out"
      onClick={() => setSidebarOpen(false)}
    >
      {/* Logo */}
      <div className="h-14 sm:h-16 flex items-center gap-2 sm:gap-3 px-3 sm:px-5 flex-shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="w-8 sm:w-9 h-8 sm:h-9 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm"
          style={{ background: 'linear-gradient(135deg, var(--brand) 0%, var(--accent) 100%)' }}>
          <Library className="w-4 sm:w-4.5 h-4 sm:h-4.5 text-white" style={{ width: 'auto', height: 'auto' }} />
        </div>
        {!collapsed && (
          <div className="min-w-0">
            <p className="text-xs sm:text-sm font-bold leading-tight" style={{ color: 'var(--text-primary)' }}>Bibliotheca</p>
            <p className="text-[8px] sm:text-[10px] leading-tight font-medium" style={{ color: 'var(--text-muted)' }}>Library System</p>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-3 sm:py-4 px-2 space-y-4 sm:space-y-5">
        {NAV.map(group => (
          <div key={group.label}>
            {!collapsed && (
              <p className="text-[8px] sm:text-[10px] font-bold uppercase tracking-widest px-3 mb-2"
      ...
      {/* Footer */}
      <div className="p-2 sm:p-3 flex items-center justify-between flex-shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
        <div
          className="flex items-center gap-2 sm:gap-3 min-w-0 px-2 sm:px-2 py-1.5 sm:py-2 rounded-xl cursor-pointer w-full"
          style={{ color: 'var(--text-primary)' }}
          onMouseEnter={e => e.currentTarget.style.backgroundColor = 'var(--bg-subtle)'}
          onMouseLeave={e => e.currentTarget.style.backgroundColor = ''}
        >
          <div className="w-6 sm:w-7 h-6 sm:h-7 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0"
          ...
          {!collapsed ? (
            <div className="flex-1 min-w-0 pl-1 flex items-center justify-between">
              <div className="min-w-0">
                <p className="text-[10px] sm:text-xs font-semibold truncate" style={{ color: 'var(--text-primary)' }}>Admin</p>
                <p className="text-[8px] sm:text-[10px]" style={{ color: 'var(--text-muted)' }}>Librarian</p>
              </div>
              <button
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: 'var(--bg-base)' }}>
      ...
      {/* Desktop sidebar */}
      <div className="hidden lg:flex flex-col relative transition-all duration-500 ease-in-out" style={{ backgroundColor: 'var(--bg-surface)', borderRight: '1px solid var(--border)' }}>
        ...
        <button
          ...
          onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--accent)'; e.currentTarget.style.transform = 'scale(1.25)'; }}
      ...
      {/* Mobile sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 lg:hidden flex flex-col transition-all duration-500 ease-in-out overflow-y-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{ width: 'min(var(--sidebar-w), 85vw)', backgroundColor: 'var(--bg-surface)', borderRight: '1px solid var(--border)' }}>
        <button className="absolute top-3 right-2 p-1.5 rounded-lg z-10" style={{ color: 'var(--text-muted)' }}
          onClick={() => setSidebarOpen(false)}>
          <X className="w-4 h-4" />
        </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden transition-all duration-500 ease-in-out">
        {/* Topbar */}
        <header className="h-14 sm:h-16 flex items-center px-3 sm:px-5 gap-3 sm:gap-4 flex-shrink-0 transition-all duration-500 ease-in-out"
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm flex-1 min-w-0">
            <Library className="w-3 sm:w-3.5 h-3 sm:h-3.5 flex-shrink-0" style={{ color: 'var(--text-muted)' }} />
            <ChevronRight className="w-2.5 sm:w-3 h-2.5 sm:h-3 flex-shrink-0" style={{ color: 'var(--text-muted)' }} />
